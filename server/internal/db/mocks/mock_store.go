...
//
//	mockgen -destination=internal/db/mocks/mock_store.go -package=mocks github.com/MattSilvaa/powhunter/internal/db StoreService
//
...
func (m *MockStoreService) CreateUserWithAlerts(ctx context.Context, email, phone string, minSnowAmount, notificationDays int32, resortUUIDs []string) error {
	m.ctrl.T.Helper()
...
func (m *MockStoreService) GetAlertMatches(ctx context.Context, resortUUID string, forecastDate time.Time, predictedSnowAmount, daysAhead int32) ([]db.AlertToSend, error) {
	m.ctrl.T.Helper()
...
func (m *MockStoreService) ListAllResorts(ctx context.Context) ([]generated.Resort, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "ListAllResorts", ctx)
	ret0, _ := ret[0].([]generated.Resort)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
