...
test('createAlert error parsing', async () => {
	...
	expect(parsedError.error).toBe('DUPLICATE_ALERT')
	expect(parsedError.message).toBe('You already have an alert for this resort')
})
...
test('AlertData type structure', () => {
	...
	expect(
		mockAlertData.resortsUuids.every((uuid) => typeof uuid === 'string')
	).toBe(true)
})
...
test('Error handling edge cases', async () => {
	...
	const textErrorResponse = new Response('Internal Server Error', {
		status: 500,
	})
	...
	const malformedJsonResponse = new Response('{"error": malformed', {
		status: 400,
	})
